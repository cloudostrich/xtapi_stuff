"""
tcm, tf, sprd = (ns.proxy(i) for i in ns.agents()[:3])
tcm.flush2disk()
tf.flush2disk()
sprd.flush2disk()
qq()
...
class InstrNotify():#getevents('XTAPI.TTInstrNotify')):
    ...
    def OnNotifyUpdate(self, pNotify=None, pInstr=None):
        ...
        upd = {'time':pd.datetime.now(), 'bidqty': bidqty, 'bid': bid, 'ask': ask,
               'askqty': askqty, 'last': last, 'lastqty': lastqty, 'contract': contract}
...
class WriteDisk(osbrain.Agent):
    ...
    def process_row(self, m, max_len=100): ##    def process_row(self, d, key, max_len=50): #, _cache=self.CACHE):
...
##            store.append(key, df)
        if os.path.isfile(self.STORE):
            with open(self.STORE,'a') as store:
                ...
                print(f"Append to Disk: {self.get_attr('name')}")
        else:
            with open(self.STORE,'w') as store:
                df.to_csv(self.STORE)
                print(f"Create New File: {self.get_attr('name')}")
        self.lst.clear()
...
def main():
    ...
    tcmw = osbrain.run_agent('wdTocom', base=WriteDisk)
    tfw = osbrain.run_agent('wdTF', base=WriteDisk)
    sprdw = osbrain.run_agent('wdSPRD', base=WriteDisk)
    Connect()
